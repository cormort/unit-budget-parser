...
`unit-budget-parser/examples/`（4 份：主計總處、農業部、交通部、衛福部 115）、
...
| W4 | `orphanRate`（中央）> 0.15 且未歸戶句中含科目名 | 未歸戶句比例偏高，**且句中寫了本科目名卻没接到** → 可改進的歸戶缺口 |
| W4b | `orphanByUnit`（中央）> 0.15 且未歸戶句不含科目名 | 這份的說明按單位／業務切、不是按科目（**資料結構限制，非 bug**；不作為門檻校準基準） |
| W5 | `descCoverage`（中央）< 0.35 | 二級科目有說明的比例偏低，可能說明整段沒讀到（若同時是 W4b 型，另標 `basis: dataLimit`） |
...
`WARN` 的數值取自**敘述按科目寫**的文件實測最差值再放寬，**刻意不拿「敘述不按科目寫」的文件校準**：

| 文件 | 未歸戶率 | 未歸戶句含本分支科目名 | 用途 |
|---|---:|---:|---|
| 行政院主計總處 | 0.3% | 0/1 | 校準基準 |
| 農業部 | 3.1% | 0/14 | 校準基準 |
| 交通部 | 0.0% | – | 校準基準 |
| 衛生福利部 | 6.9% | 85/90（94%） | 校準基準（最差值 → 門檻 15%） |
| ~~教育部~~ | ~~34.2%~~ | ~~3/450（0.7%）~~ | **不列入校準，且已不內建**：說明按單位／業務切，句中几乎不出現科目名（數字為移除前實測，保留以說明校準取捨） |

拿教育部校準會讓門檻鬆到 40%，對正常文件完全沒有鑑別力（衛福部的孤兒句從 7% 惡化到 35% 也會靜默通過）。
它是資料結構限制，不是基準——`harness.mjs` 的 `orphanShape()` 會自動分辨這兩種情形
（未歸戶句含本分支科目名的比例，樣本需 ≥ 20 句），判定結果寫進 `record.orphanShape`。

`descCoverage` 同理：符合基準的文件為 68～97%，門檻設 0.35（原本 0.05 只能抓「整欄全丟」）。
