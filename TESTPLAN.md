...
| B7 | `narrativeLost` | 有句子既沒出現在分支列總述、也沒在任何科目列說明、也沒另立未歸戶列 | 說明文字不該因為歸戶失敗而消失；被標成「已歸戶」卻無處顯示即為無聲的資料遺失 |
| B8 | `factClaim` | 白底「事實級」列的說明無法證明自己的斷言（名稱＋金額／句中加總／合併數分攤） | 工具要求使用者只信任白底，故白底的斷言必須能被機器獨立驗證；驗不過就是標籤說謊 |

B2 的判準與 B7、B8 的查核算法都只有一份實作：B2 呼叫 `index.html` 的 `_reconcileUnit()`，
B7／B8 在 `harness.mjs`，`test.mjs`／`run.mjs`／`audit.mjs` 共用。
**不要在驗證腳本裡重寫這些規則**——四層驗算曾經有過四份實作，其中 `run.mjs` 那份已漂移成
`else`（未歸戶說明列若帶 `l1Code` 就會被算進二級，產生幻影不符）。
...
| W7 | `acctCode` | 科目代碼不在官方「歲出用途別科目分類定義」清單內（不是不會被名稱校正，就是代碼讀錯；官方每年可能新增代碼，故列 warn 由人判讀） |
